# Editor play button — variants audit trail

Consultant: Claude Code (primary), one pass, 2026-06-07. Raw output:
`cli-output/claude.md` (prompt in `cli-output/prompt.md`). Variants below are
verbatim from the consultant.
...
**Rationale (verbatim)**: It best satisfies the explicit DRY mandate — reusing `applyRunStatus`, `RunNodeView` styling, and a shared poll hook instead of regrafting status onto editable nodes (Variant 1) or rebuilding orchestration at the App level (Variant 3). Making playback a read-only mode cleanly resolves the editing-while-running, dirty-spec (save-before-play gate), and `waiting`/human-review constraints by construction, and the start-poll-redirect-and-error logic stays inside one hook that the existing mocked-`WorkflowsApi` test suite can drive directly.

## Orchestrator decision

**Variant 2, accepted as recommended.** The two cons are manageable: the
renderer swap avoids the "new nodeTypes object" ReactFlow pitfall by
registering both renderers once in a single stable registry keyed by node
type, and the RunInspector refactor onto `useRunPolling` is a small extraction
covered by the existing inspector tests. Variant 1 was rejected because it
duplicates run-status rendering the project already ships, and Variant 3
because it widens App's contract for no benefit the feature needs.
